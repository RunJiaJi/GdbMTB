...
def BrowserTableGenerator(meta_df):
    ...
    colEnv = ['Assembly accession','Name','Geographic location', 'Environment', 'Geographic coordinates', 'point','Latitude', 'longitude',]
    ...
    for i in colTables:
        ...
        tablTable=dftmp.to_html(index=None, escape=False).replace(
            ...
            '<table class="table table-sm table-striped table-hover table table-fluid border" id="myTable">'
        ).replace('<thead>','<thead class="table-dark align-middle">')
